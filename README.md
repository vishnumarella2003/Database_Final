...
│   └── MySQL_Dump.sql                     # Full SQL dump of the final populated database
├── scripts/
│   ├── 01_database_exploration_cleaning.py  # Explores raw data, generates missingness reports, cleans and normalizes all six files
│   └── 02_Database_loadsql.py               # Exports cleaned data to CSVs and loads them into MySQL
├── data/
│   ├── luad_tcga_pub/                     # Original source files from cBioPortal (not modified)
│   └── luad_cleaned_staging_files/        # Cleaned CSV files used for database loading
├── docs/
│   ├── Database_Documentation_Final.pdf   # Full design documentation and decision rationale
...
> **Note:** Raw data files are not included in this repository due to size. Download them directly from [cBioPortal](https://www.cbioportal.org/) and place them in `data/luad_tcga_pub/` before running any scripts.
...
Download the six TCGA LUAD files from [cBioPortal](https://www.cbioportal.org/) and place them in `data/luad_tcga_pub/`.

### 3. Run the Python scripts in order

```bash
python scripts/01_database_exploration_cleaning.py   # Explores, cleans, and normalizes all six source files
python scripts/02_Database_loadsql.py                # Exports cleaned CSVs and loads them into MySQL
```
...
```bash
mysql -u your_username -p lung_cancer_db < sql/MySQL_Dump.sql
```
...
| Resource | Location |
|---|---|
| Full project write-up | `docs/Database_Documentation_Final.pdf` |
